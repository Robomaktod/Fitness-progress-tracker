...
const ProfileScreen: React.FC = () => {
  ...
  function mapBackendUserToFrontendProfile(backendUser: any): FrontendUserProfileData {
    // Use latest weight log for currentWeight
    const latestWeightLog = backendUser.weightLogs?.length
      ? backendUser.weightLogs.reduce((a: any, b: any) => (new Date(a.loggedAt) > new Date(b.loggedAt) ? a : b))
      : null;
    ...
    const age = backendUser.dateOfBirth ? Math.floor((Date.now() - new Date(backendUser.dateOfBirth).getTime()) / (365.25 * 24 * 60 * 60 * 1000)) : null;
    // Calculate daily norma (BMR * 1.2)
    let dailyNorma = null;
    if (backendUser.heightCm && latestWeight && backendUser.dateOfBirth && backendUser.gender) {
      const now = new Date();
      const birth = new Date(backendUser.dateOfBirth);
      const userAge = now.getFullYear() - birth.getFullYear() - (now < new Date(birth.setFullYear(now.getFullYear())) ? 1 : 0);
      const s = backendUser.gender === 'Male' ? 5 : -161;
      const bmr = 10 * Number(latestWeight) + 6.25 * Number(backendUser.heightCm) - 5 * userAge + s;
    ...
    return {
      ...
      memberSince: backendUser.createdAt ? new Date(backendUser.createdAt).toLocaleDateString() : "",
      bodyMetrics: {
        currentWeight: { id: "currentWeight", label: "Current Weight", value: latestWeight ? latestWeight.toString() : "-", unit: "kg", valueClassName: "text-cyan-400" },
        goalWeight: { id: "goalWeight", label: "Goal Weight", value: goalWeight ? goalWeight.toString() : "-", unit: "kg", valueClassName: "text-purple-400" },
        height: { id: "height", label: "Height", value: backendUser.heightCm?.toString() || "-", unit: "cm", valueClassName: "text-blue-400" },
        age: { id: "age", label: "Age", value: age ? age.toString() : "-", unit: "yrs", valueClassName: "text-green-400" },
      },
      fitnessGoals: backendUser.fitnessGoals || [], // Only if you add this to your schema
      dailyTargets: {
        calories: { id: "calories", label: "Calories", value: dailyNorma ? dailyNorma.toString() : "-", unit: "kcal", valueClassName: "text-blue-400" },
        protein: { id: "protein", label: "", value: "", unit: "", valueClassName: "" },
        carbs: { id: "carbs", label: "", value: "", unit: "", valueClassName: "" },
  ...
  const handleEditProfile = () => {
    ...
    setEditForm({
      ...
      currentWeight: user.weightLogs?.length ? user.weightLogs.reduce((a: any, b: any) => (new Date(a.loggedAt) > new Date(b.loggedAt) ? a : b)).weightKg : "",
  ...
  const handleSaveEdit = () => {
    updateUserProfile({ userId: userId ?? "", update: editForm }, {
      onSuccess: () => {
      ...
      <SafeAreaView edges={["top"]} className="flex-1">
        ...
        <Modal visible={editVisible} animationType="slide" transparent>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', alignItems: 'center' }}>
            <View style={{ backgroundColor: '#222', padding: 24, borderRadius: 16, width: '90%' }}>
              <Text className="text-lg font-bold text-white mb-4">Edit Profile</Text>
              <TextInput
                placeholder="Name"
                value={editForm.name}
                onChangeText={v => setEditForm((f: any) => ({ ...f, name: v }))}
                className="bg-dark-100 text-white mb-2 p-2 rounded"
                placeholderTextColor="#aaa"
              />
              <TextInput
                placeholder="Height (cm)"
                value={editForm.heightCm?.toString()}
                onChangeText={v => setEditForm((f: any) => ({ ...f, heightCm: v }))}
              ...
              <TextInput
                ...
                onChangeText={v => setEditForm((f: any) => ({ ...f, dateOfBirth: v }))}
                className="bg-dark-100 text-white mb-2 p-2 rounded"
                placeholderTextColor="#aaa"
              />
              <TextInput
                ...
                onChangeText={v => setEditForm((f: any) => ({ ...f, currentWeight: v }))}
                keyboardType="numeric"
                className="bg-dark-100 text-white mb-2 p-2 rounded"
                placeholderTextColor="#aaa"
              />
              <TextInput
                ...
                onChangeText={v => setEditForm((f: any) => ({ ...f, goalWeight: v }))}
                keyboardType="numeric"
                className="bg-dark-100 text-white mb-2 p-2 rounded"
                placeholderTextColor="#aaa"
              />
              ...
              <TextInput
                ...
                value={Array.isArray(editForm.fitnessGoals) ? editForm.fitnessGoals.map((g: any) => g.value || g).join(", ") : ""}
                onChangeText={v => setEditForm((f: any) => ({ ...f, fitnessGoals: v.split(",").map((g: string) => ({ id: g.trim(), label: g.trim(), value: g.trim() })) }))}
                className="bg-dark-100 text-white mb-2 p-2 rounded"
                placeholderTextColor="#aaa"
              />
              <View className="flex-row justify-between mt-4">
                <CustomButton title="Cancel" onPress={() => setEditVisible(false)} className="flex-1 mr-2 bg-gray-700" isGradientActive={false} />
                <CustomButton title="Save" onPress={handleSaveEdit} className="flex-1 ml-2 bg-purple-600" isGradientActive={false} />
              </View>
            </View>
          </View>
        </Modal>
