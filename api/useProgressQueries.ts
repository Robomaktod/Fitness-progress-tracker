...
export function useActivityTrends(userId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get(`/progress/activity?userId=${userId}`);
      return data;
    },
    enabled: !!userId,
  });
...
export function useCalorieTrends(userId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get(`/progress/calories?userId=${userId}`);
      return data;
    },
    enabled: !!userId,
  });
...
export function useAddWeight(userId: string, options?: any) {
  return useMutation<
    any, // response type (can be more specific)
    Error,
